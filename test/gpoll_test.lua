require('luacov')
local assert = require('assert')
local errno = require('errno')
local fork = require('fork')
local getpid = require('getpid')
local msleep = require('nanosleep.msleep')
local signal = require('signal')
local gpoll = require('gpoll')
local TMPFILE = assert(io.tmpfile())
local TMPFD = require('io.fileno')(TMPFILE)
local DEFAULT_FUNCS = {}
for name, fn in pairs(gpoll) do
    DEFAULT_FUNCS[name] = fn
end

local function do_set_poller(funcs)
    local new_funcs = {}
    for name, fn in pairs(DEFAULT_FUNCS) do
        new_funcs[name] = funcs[name] or fn
    end
    gpoll.set_poller(new_funcs)
end

local function test_default()
    -- test that default returns
    local ok, err = gpoll.pollable()
    assert.is_false(ok)
    assert.is_nil(err)

    ok, err = gpoll.later()
    assert.is_true(ok)
    assert.is_nil(err)

    -- new readable event
    local evid
    evid, err = gpoll.new_readable_event(1)
    assert.is_nil(evid)
    assert.equal(err.type, errno.ENOTSUP)

    -- new writable event
    evid, err = gpoll.new_writable_event(1)
    assert.is_nil(evid)
    assert.equal(err.type, errno.ENOTSUP)

    -- wait event
    ok, err = gpoll.wait_event('foo', 1)
    assert.is_false(ok)
    assert.equal(err.type, errno.ENOTSUP)

    -- dispose event
    ok, err = gpoll.dispose_event('foo')
    assert.is_false(ok)
    assert.equal(err.type, errno.ENOTSUP)

    local timeout
    for _, fn in ipairs({
        gpoll.unwait,
        gpoll.unwait_readable,
        gpoll.unwait_writable,
        gpoll.read_lock,
        gpoll.write_lock,
        gpoll.read_unlock,
        gpoll.write_unlock,
        gpoll.wait_readable,
        gpoll.wait_writable,
    }) do
        ok, err, timeout = fn(1)
        assert.is_false(ok)
        assert.equal(err.type, errno.ENOTSUP)
        assert.is_nil(timeout)
    end

    -- wait signal
    local signo
    signo, err, timeout = gpoll.sigwait(100, signal.SIGINT)
    assert.is_nil(signo)
    assert.equal(err.type, errno.ENOTSUP)
    assert.is_nil(timeout)

    -- wait sleep
    local rem
    rem, err = gpoll.sleep(100)
    assert.is_nil(rem)
    assert.equal(err.type, errno.ENOTSUP)
end

local function test_set_poller()
    -- test that set custom poller
    do_set_poller({
        pollable = function()
            return true
        end,
    })
    assert.is_true(gpoll.pollable())

    -- test that poller can be reset to default with nil
    gpoll.set_poller()
    assert.is_false(gpoll.pollable())

    -- test that throws an error if poller api is not function
    local err = assert.throws(gpoll.set_poller, {
        pollable = 'pollable',
        later = 'later',
        wait_readable = 'wait_readable',
        wait_writable = 'wait_writable',
        unwait = 'unwait',
        unwait_readable = 'unwait_readable',
        unwait_writable = 'unwait_writable',
        read_lock = 'read_lock',
        write_lock = 'write_lock',
        read_unlock = 'read_unlock',
        write_unlock = 'write_unlock',
        sleep = 'sleep',
        sigwait = 'sigwait',
    })
    assert.match(err, 'is not function')
end

local function test_wait_readable()
    gpoll.set_poller()

    -- test that true if fd is wait readable
    local ok, err, timeout = gpoll.wait_readable(TMPFD)
    assert.is_false(ok)
    assert.equal(err.type, errno.ENOTSUP)
    assert.is_nil(timeout)

    -- test that hookfn is called before waiting for fd event
    local called = false
    ok, err, timeout = gpoll.wait_readable(TMPFD, nil, function()
        called = true
        return true
    end)
    assert.is_false(ok)
    assert.equal(err.type, errno.ENOTSUP)
    assert.is_nil(timeout)
    assert.is_true(called)

    -- test that return error from hookfn
    ok, err, timeout = gpoll.wait_readable(TMPFD, nil, function()
        return false, 'hook error', true
    end)
    assert.is_false(ok)
    assert.match(err, 'hook error')
    assert.is_true(timeout)

    -- test that return timeout from hookfn
    ok, err, timeout = gpoll.wait_readable(TMPFD, nil, function()
        return false, nil, true
    end)
    assert.is_false(ok)
    assert.is_nil(err)
    assert.is_true(timeout)

    -- test that return only true
    do_set_poller({
        wait_readable = function()
            return true, 'this error is ignored', true
        end,
    })
    ok, err, timeout = gpoll.wait_readable(TMPFD)
    assert.is_true(ok)
    assert.is_nil(err)
    assert.is_nil(timeout)

    -- test that return error
    do_set_poller({
        wait_readable = function()
            return false, 'wait error', true
        end,
    })
    ok, err, timeout = gpoll.wait_readable(TMPFD)
    assert.is_false(ok)
    assert.match(err, 'wait error')
    assert.is_true(timeout)

    -- test that return timeout
    do_set_poller({
        wait_readable = function()
            return false, nil, true
        end,
    })
    ok, err, timeout = gpoll.wait_readable(TMPFD)
    assert.is_false(ok)
    assert.is_nil(err)
    assert.is_true(timeout)

    -- test that throws an error if fd is invalid
    err = assert.throws(gpoll.wait_readable, 'hello')
    assert.match(err, 'fd must be uint')

    -- test that throws an error if msec is invalid
    err = assert.throws(gpoll.wait_readable, TMPFD, 'hello')
    assert.match(err, 'msec must be uint')

    -- test that throws an error if hookfn is invalid
    err = assert.throws(gpoll.wait_readable, TMPFD, nil, {})
    assert.match(err, 'hookfn must be function')

    -- test that throws an error if hookfn return false with neither error nor timeout
    err = assert.throws(gpoll.wait_readable, TMPFD, nil, function()
        return false
    end)
    assert.match(err, 'hookfn .+ neither error nor timeout', false)

    -- test that throws an error if wait_readable return false with neither error nor timeout
    do_set_poller({
        wait_readable = function()
            return false
        end,
    })
    err = assert.throws(gpoll.wait_readable, TMPFD)
    assert.match(err, 'wait_readable .+ neither error nor timeout', false)
end

local function test_wait_writable()
    gpoll.set_poller()

    -- test that true if fd is wait writable
    local ok, err, timeout = gpoll.wait_writable(TMPFD)
    assert.is_false(ok)
    assert.equal(err.type, errno.ENOTSUP)
    assert.is_nil(timeout)

    -- test that hookfn is called before waiting for fd event
    local called = false
    ok, err, timeout = gpoll.wait_writable(TMPFD, nil, function()
        called = true
        return true
    end)
    assert.is_false(ok)
    assert.equal(err.type, errno.ENOTSUP)
    assert.is_nil(timeout)
    assert.is_true(called)

    -- test that return error from hookfn
    ok, err, timeout = gpoll.wait_writable(TMPFD, nil, function()
        return false, 'hook error'
    end)
    assert.is_false(ok)
    assert.match(err, 'hook error')
    assert.is_nil(timeout)

    -- test that return timeout from hookfn
    ok, err, timeout = gpoll.wait_writable(TMPFD, nil, function()
        return false, nil, true
    end)
    assert.is_false(ok)
    assert.is_nil(err)
    assert.is_true(timeout)

    -- test that return only true
    do_set_poller({
        wait_writable = function()
            return true, 'this error is ignored', true
        end,
    })
    ok, err, timeout = gpoll.wait_writable(TMPFD)
    assert.is_true(ok)
    assert.is_nil(err)
    assert.is_nil(timeout)

    -- test that return error and timeout
    do_set_poller({
        wait_writable = function()
            return false, 'wait error', true
        end,
    })
    ok, err, timeout = gpoll.wait_writable(TMPFD)
    assert.is_false(ok)
    assert.match(err, 'wait error')
    assert.is_true(timeout)

    -- test that return timeout
    do_set_poller({
        wait_writable = function()
            return false, nil, true
        end,
    })
    ok, err, timeout = gpoll.wait_writable(TMPFD)
    assert.is_false(ok)
    assert.is_nil(err)
    assert.is_true(timeout)

    -- test that throws an error if fd is invalid
    err = assert.throws(gpoll.wait_writable, 'hello')
    assert.match(err, 'fd must be uint')

    -- test that throws an error if msec is invalid
    err = assert.throws(gpoll.wait_writable, TMPFD, 'hello')
    assert.match(err, 'msec must be uint')

    -- test that throws an error if hookfn is invalid
    err = assert.throws(gpoll.wait_writable, TMPFD, nil, {})
    assert.match(err, 'hookfn must be function')

    -- test that throws an error if hookfn return false with neither error nor timeout
    err = assert.throws(gpoll.wait_writable, TMPFD, nil, function()
        return false
    end)
    assert.match(err, 'hookfn .+ neither error nor timeout', false)

    -- test that throws an error if wait_writable return false with neither error nor timeout
    do_set_poller({
        wait_writable = function()
            return false
        end,
    })
    err = assert.throws(gpoll.wait_writable, TMPFD)
    assert.match(err, 'wait_writable .+ neither error nor timeout', false)
end

local function test_unwait_readable()
    gpoll.set_poller()

    -- test that true if fd is unwait readable
    local ok, err = gpoll.unwait_readable(TMPFD)
    assert.is_false(ok)
    assert.equal(err.type, errno.ENOTSUP)

    -- test that return only true
    do_set_poller({
        unwait_readable = function()
            return true, 'this error is ignored'
        end,
    })
    ok, err = gpoll.unwait_readable(TMPFD)
    assert.is_true(ok)
    assert.is_nil(err)

    -- test that return error
    do_set_poller({
        unwait_readable = function()
            return false, 'unwait error'
        end,
    })
    ok, err = gpoll.unwait_readable(TMPFD)
    assert.is_false(ok)
    assert.match(err, 'unwait error')

    -- test that throws an error if fd is invalid
    err = assert.throws(gpoll.unwait_readable, 'hello')
    assert.match(err, 'fd must be uint')

    -- test that throws an error if unwait_readable return false without error
    do_set_poller({
        unwait_readable = function()
            return false
        end,
    })
    err = assert.throws(gpoll.unwait_readable, TMPFD)
    assert.match(err, 'unwait_readable .+ without an error', false)
end

local function test_unwait_writable()
    gpoll.set_poller()

    -- test that true if fd is unwait writable
    local ok, err = gpoll.unwait_writable(TMPFD)
    assert.is_false(ok)
    assert.equal(err.type, errno.ENOTSUP)

    -- test that return only true
    do_set_poller({
        unwait_writable = function()
            return true, 'this error is ignored'
        end,
    })
    ok, err = gpoll.unwait_writable(TMPFD)
    assert.is_true(ok)
    assert.is_nil(err)

    -- test that return error
    do_set_poller({
        unwait_writable = function()
            return false, 'unwait error'
        end,
    })
    ok, err = gpoll.unwait_writable(TMPFD)
    assert.is_false(ok)
    assert.match(err, 'unwait error')

    -- test that throws an error if fd is invalid
    err = assert.throws(gpoll.unwait_writable, 'hello')
    assert.match(err, 'fd must be uint')

    -- test that throws an error if unwait_writable return false without error
    do_set_poller({
        unwait_writable = function()
            return false
        end,
    })
    err = assert.throws(gpoll.unwait_writable, TMPFD)
    assert.match(err, 'unwait_writable .+ without an error', false)
end

local function test_unwait()
    gpoll.set_poller()

    -- test that true if fd is unwait readable/writable
    local ok, err = gpoll.unwait(TMPFD)
    assert.is_false(ok)
    assert.equal(err.type, errno.ENOTSUP)

    -- test that return only true
    do_set_poller({
        unwait = function()
            return true, 'this error is ignored'
        end,
    })
    ok, err = gpoll.unwait(TMPFD)
    assert.is_true(ok)
    assert.is_nil(err)

    -- test that return error
    do_set_poller({
        unwait = function()
            return false, 'unwait error'
        end,
    })
    ok, err = gpoll.unwait(TMPFD)
    assert.is_false(ok)
    assert.match(err, 'unwait error')

    -- test that throws an error if fd is invalid
    err = assert.throws(gpoll.unwait, 'hello')
    assert.match(err, 'fd must be uint')

    -- test that throws an error if unwait return false without error
    do_set_poller({
        unwait = function()
            return false
        end,
    })
    err = assert.throws(gpoll.unwait, TMPFD)
    assert.match(err, 'unwait .+ without an error', false)
end

local function test_read_lock()
    gpoll.set_poller()

    -- test that true if fd is read_lock
    local ok, err, timeout = gpoll.read_lock(TMPFD)
    assert.is_false(ok)
    assert.equal(err.type, errno.ENOTSUP)
    assert.is_nil(timeout)

    -- test that return only true
    do_set_poller({
        read_lock = function()
            return true, 'this error is ignored', true
        end,
    })
    ok, err, timeout = gpoll.read_lock(TMPFD)
    assert.is_true(ok)
    assert.is_nil(err)
    assert.is_nil(timeout)

    -- test that return error and timeout
    do_set_poller({
        read_lock = function()
            return false, 'lock error', true
        end,
    })
    ok, err, timeout = gpoll.read_lock(TMPFD)
    assert.is_false(ok)
    assert.match(err, 'lock error')
    assert.is_true(timeout)

    -- test that return timeout
    do_set_poller({
        read_lock = function()
            return false, nil, true
        end,
    })
    ok, err, timeout = gpoll.read_lock(TMPFD)
    assert.is_false(ok)
    assert.is_nil(err)
    assert.is_true(timeout)

    -- test that throws an error if fd is invalid
    err = assert.throws(gpoll.read_lock, 'hello')
    assert.match(err, 'fd must be uint')

    -- test that throws an error if msec is invalid
    err = assert.throws(gpoll.read_lock, TMPFD, 'hello')
    assert.match(err, 'msec must be uint')

    -- test that throws an error if read_lock return false with neither error nor timeout
    do_set_poller({
        read_lock = function()
            return false
        end,
    })
    err = assert.throws(gpoll.read_lock, TMPFD)
    assert.match(err, 'read_lock .+ neither error nor timeout', false)
end

local function test_write_lock()
    gpoll.set_poller()

    -- test that true if fd is write_lock
    local ok, err, timeout = gpoll.write_lock(TMPFD)
    assert.is_false(ok)
    assert.equal(err.type, errno.ENOTSUP)
    assert.is_nil(timeout)

    -- test that return only true
    do_set_poller({
        write_lock = function()
            return true, 'this error is ignored', true
        end,
    })
    ok, err, timeout = gpoll.write_lock(TMPFD)
    assert.is_true(ok)
    assert.is_nil(err)
    assert.is_nil(timeout)

    -- test that return error and timeout
    do_set_poller({
        write_lock = function()
            return false, 'lock error', true
        end,
    })
    ok, err, timeout = gpoll.write_lock(TMPFD)
    assert.is_false(ok)
    assert.match(err, 'lock error')
    assert.is_true(timeout)

    -- test that return timeout
    do_set_poller({
        write_lock = function()
            return false, nil, true
        end,
    })
    ok, err, timeout = gpoll.write_lock(TMPFD)
    assert.is_false(ok)
    assert.is_nil(err)
    assert.is_true(timeout)

    -- test that throws an error if fd is invalid
    err = assert.throws(gpoll.write_lock, 'hello')
    assert.match(err, 'fd must be uint')

    -- test that throws an error if msec is invalid
    err = assert.throws(gpoll.write_lock, TMPFD, 'hello')
    assert.match(err, 'msec must be uint')

    -- test that throws an error if write_lock return false with neither error nor timeout
    do_set_poller({
        write_lock = function()
            return false
        end,
    })
    err = assert.throws(gpoll.write_lock, TMPFD)
    assert.match(err, 'write_lock .+ neither error nor timeout', false)
end

local function test_read_unlock()
    gpoll.set_poller()
    -- test that true if fd is read_unlock
    local ok, err = gpoll.read_unlock(TMPFD)
    assert.is_false(ok)
    assert.equal(err.type, errno.ENOTSUP)

    -- test that return only true
    do_set_poller({
        read_unlock = function()
            return true, 'this error is ignored'
        end,
    })
    ok, err = gpoll.read_unlock(TMPFD)
    assert.is_true(ok)
    assert.is_nil(err)

    -- test that return error
    do_set_poller({
        read_unlock = function()
            return false, 'unlock error'
        end,
    })
    ok, err = gpoll.read_unlock(TMPFD)
    assert.is_false(ok)
    assert.match(err, 'unlock error')

    -- test that throws an error if fd is invalid
    err = assert.throws(gpoll.read_unlock, 'hello')
    assert.match(err, 'fd must be uint')

    -- test that throws an error if read_unlock return false without error
    do_set_poller({
        read_unlock = function()
            return false
        end,
    })
    err = assert.throws(gpoll.read_unlock, TMPFD)
    assert.match(err, 'read_unlock .+ without an error', false)
end

local function test_write_unlock()
    gpoll.set_poller()

    -- test that true if fd is write_unlock
    local ok, err = gpoll.write_unlock(TMPFD)
    assert.is_false(ok)
    assert.equal(err.type, errno.ENOTSUP)

    -- test that return only true
    do_set_poller({
        write_unlock = function()
            return true, 'this error is ignored'
        end,
    })
    ok, err = gpoll.write_unlock(TMPFD)
    assert.is_true(ok)
    assert.is_nil(err)

    -- test that return error
    do_set_poller({
        write_unlock = function()
            return false, 'unlock error'
        end,
    })
    ok, err = gpoll.write_unlock(TMPFD)
    assert.is_false(ok)
    assert.match(err, 'unlock error')

    -- test that throws an error if fd is invalid
    err = assert.throws(gpoll.write_unlock, 'hello')
    assert.match(err, 'fd must be uint')

    -- test that throws an error if write_lock return false without error
    do_set_poller({
        write_unlock = function()
            return false
        end,
    })
    err = assert.throws(gpoll.write_unlock, TMPFD)
    assert.match(err, 'write_unlock .+ without an error', false)
end

local function test_sleep()
    -- test that sleep for 1 sec
    local rem, err = gpoll.sleep(1000)
    assert.is_nil(rem)
    assert.equal(err.type, errno.ENOTSUP)

    -- test that set custome sleep function
    do_set_poller({
        sleep = function()
            return 2
        end,
    })
    rem, err = gpoll.sleep(1)
    assert.equal(rem, 2)
    assert.is_nil(err)

    -- test that return error
    do_set_poller({
        sleep = function()
            return nil, 'sleep error'
        end,
    })
    rem, err = gpoll.sleep(1)
    assert.is_nil(rem)
    assert.match(err, 'sleep error')

    -- test that throws an error if msec is invalid
    err = assert.throws(gpoll.sleep, math.huge)
    assert.match(err, 'msec must be uint')

    -- test that throws an error if sleep return non-uint value
    do_set_poller({
        sleep = function()
            return math.huge
        end,
    })
    err = assert.throws(gpoll.sleep, 1)
    assert.match(err, 'sleep returned non-uint value')

    -- test that throws an error if sleep return nil without an error
    do_set_poller({
        sleep = function()
            return nil
        end,
    })
    err = assert.throws(gpoll.sleep, 1)
    assert.match(err, 'sleep returned nil without an error')
end

local function test_sigwait()
    -- test that wait signal
    local pid = getpid()
    local p = assert(fork())
    if p:is_child() then
        msleep(200)
        signal.kill(signal.SIGINT, pid)
        os.exit(0)
    end
    local signo, err, timeout = gpoll.sigwait(500, signal.SIGINT)
    assert.is_nil(signo)
    assert.equal(err.type, errno.ENOTSUP)
    assert.is_nil(timeout)

    -- test that timeout
    signo, err, timeout = signal.wait(100, signal.SIGINT)
    assert.is_nil(signo)
    assert.is_nil(err)
    assert.is_true(timeout)

    -- test that return only true
    do_set_poller({
        sigwait = function()
            return 123, 'this error is ignored', true
        end,
    })
    signo, err, timeout = gpoll.sigwait(100, signal.SIGINT)
    assert.equal(signo, 123)
    assert.is_nil(err)
    assert.is_nil(timeout)

    -- test that return error and timeout
    do_set_poller({
        sigwait = function()
            return nil, 'sigwait error', true
        end,
    })
    signo, err, timeout = gpoll.sigwait(100)
    assert.is_nil(signo)
    assert.match(err, 'sigwait error')
    assert.is_true(timeout)

    -- test that return timeout
    do_set_poller({
        sigwait = function()
            return nil, nil, true
        end,
    })
    signo, err, timeout = gpoll.sigwait(100)
    assert.is_nil(signo)
    assert.is_nil(err)
    assert.is_true(timeout)

    -- test that throws an error if msec is invalid
    err = assert.throws(gpoll.sigwait, math.huge)
    assert.match(err, 'msec must be uint')

    -- test that throws an error if sigwait return non-int value
    do_set_poller({
        sigwait = function()
            return math.huge
        end,
    })
    err = assert.throws(gpoll.sigwait, 1)
    assert.match(err, 'sigwait returned non-int value')

    -- test that throws an error if sigwait return nil without an error
    do_set_poller({
        sigwait = function()
            return nil
        end,
    })
    err = assert.throws(gpoll.sigwait, 1)
    assert.match(err, 'sigwait returned nil with neither error nor timeout')
end

local function test_readable_event()
    gpoll.set_poller()

    -- test that return error by default
    local evid, err = gpoll.new_readable_event(TMPFD)
    assert.is_nil(evid)
    assert.equal(err.type, errno.ENOTSUP)

    -- test that return only evid
    do_set_poller({
        new_readable_event = function()
            return 123, 'this error is ignored'
        end,
    })
    evid, err = gpoll.new_readable_event(TMPFD)
    assert.equal(evid, 123)
    assert.is_nil(err)

    -- test that return error
    do_set_poller({
        new_readable_event = function()
            return nil, 'new event error'
        end,
    })
    evid, err = gpoll.new_readable_event(TMPFD)
    assert.is_nil(evid)
    assert.match(err, 'new event error')

    -- test that throws an error if fd is invalid
    err = assert.throws(gpoll.new_readable_event, 'hello')
    assert.match(err, 'fd must be uint')

    -- test that throws an error if it return nil with no error
    do_set_poller({
        new_readable_event = function()
            return nil
        end,
    })
    err = assert.throws(gpoll.new_readable_event, TMPFD)
    assert.match(err, 'new_readable_event .+ with no error', false)
end

local function test_writable_event()
    gpoll.set_poller()

    -- test that return error by default
    local evid, err = gpoll.new_writable_event(TMPFD)
    assert.is_nil(evid)
    assert.equal(err.type, errno.ENOTSUP)

    -- test that return only evid
    do_set_poller({
        new_writable_event = function()
            return 123, 'this error is ignored'
        end,
    })
    evid, err = gpoll.new_writable_event(TMPFD)
    assert.equal(evid, 123)
    assert.is_nil(err)

    -- test that return error
    do_set_poller({
        new_writable_event = function()
            return nil, 'new event error'
        end,
    })
    evid, err = gpoll.new_writable_event(TMPFD)
    assert.is_nil(evid)
    assert.match(err, 'new event error')

    -- test that throws an error if fd is invalid
    err = assert.throws(gpoll.new_writable_event, 'hello')
    assert.match(err, 'fd must be uint')

    -- test that throws an error if it return nil with no error
    do_set_poller({
        new_writable_event = function()
            return nil
        end,
    })
    err = assert.throws(gpoll.new_writable_event, TMPFD)
    assert.match(err, 'new_writable_event .+ with no error', false)
end

local function test_dispose_event()
    gpoll.set_poller()

    -- test that return error by default
    local ok, err = gpoll.dispose_event(123)
    assert.is_false(ok)
    assert.equal(err.type, errno.ENOTSUP)

    -- test that return only true
    do_set_poller({
        dispose_event = function()
            return true, 'this error is ignored'
        end,
    })
    ok, err = gpoll.dispose_event(123)
    assert.is_true(ok)
    assert.is_nil(err)

    -- test that return error
    do_set_poller({
        dispose_event = function()
            return nil, 'dispose error'
        end,
    })
    ok, err = gpoll.dispose_event(123)
    assert.is_false(ok)
    assert.match(err, 'dispose error')

    -- test that throws an error if evid is nil
    err = assert.throws(gpoll.dispose_event, nil)
    assert.match(err, 'evid must not be nil')

    -- test that throws an error if it return nil with no error
    do_set_poller({
        dispose_event = function()
            return nil
        end,
    })
    err = assert.throws(gpoll.dispose_event, TMPFD)
    assert.match(err, 'dispose_event .+ with no error', false)
end

local function test_wait_event()
    gpoll.set_poller()

    -- test that return error by default
    local ok, err, timeout = gpoll.wait_event(123)
    assert.is_false(ok)
    assert.equal(err.type, errno.ENOTSUP)
    assert.is_nil(timeout)

    -- test that hookfn is called before waiting for event
    local called = false
    ok, err, timeout = gpoll.wait_event(123, nil, function()
        called = true
        return true
    end)
    assert.is_false(ok)
    assert.equal(err.type, errno.ENOTSUP)
    assert.is_nil(timeout)
    assert.is_true(called)

    -- test that return error and timeout from hookfn
    ok, err, timeout = gpoll.wait_event(123, nil, function()
        return false, 'hook error', true
    end)
    assert.is_false(ok)
    assert.match(err, 'hook error')
    assert.is_true(timeout)

    -- test that return timeout from hookfn
    ok, err, timeout = gpoll.wait_event(123, nil, function()
        return false, nil, true
    end)
    assert.is_false(ok)
    assert.is_nil(err)
    assert.is_true(timeout)

    -- test that return only true
    do_set_poller({
        wait_event = function()
            return true, 'this error is ignored', true
        end,
    })
    ok, err, timeout = gpoll.wait_event(123)
    assert.is_true(ok)
    assert.is_nil(err)
    assert.is_nil(timeout)

    -- test that return error and timeout
    do_set_poller({
        wait_event = function()
            return false, 'wait error', true
        end,
    })
    ok, err, timeout = gpoll.wait_event(TMPFD)
    assert.is_false(ok)
    assert.match(err, 'wait error')
    assert.is_true(timeout)

    -- test that return timeout
    do_set_poller({
        wait_event = function()
            return false, nil, true
        end,
    })
    ok, err, timeout = gpoll.wait_event(TMPFD)
    assert.is_false(ok)
    assert.is_nil(err)
    assert.is_true(timeout)

    -- test that throws an error if evid is nil
    err = assert.throws(gpoll.wait_event, nil)
    assert.match(err, 'evid must not be nil')

    -- test that throws an error if msec is invalid
    err = assert.throws(gpoll.wait_event, 123, 'hello')
    assert.match(err, 'msec must be uint')

    -- test that throws an error if hookfn is invalid
    err = assert.throws(gpoll.wait_event, 123, nil, {})
    assert.match(err, 'hookfn must be function')

    -- test that throws an error if hookfn return false with neither error nor timeout
    err = assert.throws(gpoll.wait_event, 123, nil, function()
        return false
    end)
    assert.match(err, 'hookfn .+ neither error nor timeout', false)

    -- test that throws an error if wait_event return false with neither error nor timeout
    do_set_poller({
        wait_event = function()
            return false
        end,
    })
    err = assert.throws(gpoll.wait_event, 123)
    assert.match(err, 'wait_event .+ neither error nor timeout', false)
end

for k, f in pairs({
    test_default = test_default,
    test_set_poller = test_set_poller,
    test_wait_readable = test_wait_readable,
    test_wait_writable = test_wait_writable,
    test_unwait_readable = test_unwait_readable,
    test_unwait_writable = test_unwait_writable,
    test_unwait = test_unwait,
    test_read_lock = test_read_lock,
    test_write_lock = test_write_lock,
    test_read_unlock = test_read_unlock,
    test_write_unlock = test_write_unlock,
    test_sleep = test_sleep,
    test_sigwait = test_sigwait,
    test_readable_event = test_readable_event,
    test_writable_event = test_writable_event,
    test_dispose_event = test_dispose_event,
    test_wait_event = test_wait_event,
}) do
    gpoll.set_poller()
    local ok, err = xpcall(f, debug.traceback)
    if ok then
        print(k .. ': ok')
    else
        print(k .. ': fail')
        print(err)
    end
end

